...
echo "Please install java $JAVA_VERSION or newer"
echo "***********************************"
...
echo "This software needs java $JAVA_VERSION or newer."
echo "***********************************"
else
# Java version OK
...
#--------------------------
CLASSPATH=
CLASSPATH=$CLASSPATH:$JAR_ROOT/fr.soleil.gui-image-multiplication-1.2.2-3627.jar
...
JAVA_BITS=`java -version 2>&1 | grep 64-Bit`
if [ "$JAVA_BITS" = "" ]
then
# 32bits JVM
NEXUS_PATH=$TOP_DIR/native/linux32
else
# 64bits JVM
NEXUS_PATH=$TOP_DIR/native/linux64
fi
...
fr.soleil.image.multiplication.ImageMultiplication $@
fi
